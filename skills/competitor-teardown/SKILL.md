...
description: "Produces a deep, evidence-backed intelligence report on one or more named competitors using the ScrapingDog MCP server — product and pricing scraped rather than guessed, hiring velocity from live job postings, funding and press recency, review volume and sentiment, and social reach — then computes a deterministic 0-100 momentum score per competitor so a founder can see who is actually growing versus who is stalling, not just who has the nicest homepage. Second stage of a three-skill pipeline: startup-idea-validator surfaces the competitor names, and gtm-launch-planner turns the gaps found here into a launch plan. Use when the user names specific competitors and wants to understand them deeply — 'tell me about CodeRabbit and Greptile', 'who is winning between X and Y', 'is [competitor] actually growing', 'what is [competitor] pricing or weakness' — or right after a validator run when they want to go deeper on the top rivals. For a first-pass 'who are the competitors' scan, use startup-idea-validator instead."
---
...
- [ ] Every headcount, review quote, funding date, and job-posting count in your notes traces to a
      specific tool call, not memory.
- [ ] Every competitor with no findable signal in a category has that recorded as "not found," not
      left blank or guessed at.
- [ ] The band you're about to write matches what `momentum_score.py` actually returned for that
      competitor.
...
- **Never invent**: a headcount, review quote, funding date, or job-posting count. A competitor with
  genuinely no findable signal in a category gets "not found," not a plausible-sounding guess. The
  tell: a specific number or quote in the report that isn't backed by a tool result in your notes.
- **Momentum isn't quality**: the score describes trajectory signals (hiring, news, reviews, reach),
  not product quality. A well-funded competitor with a mediocre product can still score high
  momentum. Say so in the report rather than implying the score means "best."
- **Depth over list length**: cap scope at the competitors that matter. This skill rewards depth per
  competitor, not a long list of shallow entries.
