...
description: "Validates a startup, SaaS, app, product, or side-project idea using live web data pulled through the ScrapingDog MCP server (search, trends, news, jobs, marketplaces, LinkedIn, X, YouTube, Maps, and a direct ChatGPT-query tool) — finds real competitors, demand and trend signals, pricing benchmarks, AI-answer-engine visibility, and verbatim audience pain points, then scores the idea 0-100 and delivers a go/no-go verdict as a shareable report. First stage of a three-skill pipeline, ahead of competitor-teardown and gtm-launch-planner. Use whenever the user wants to validate, vet, sanity-check, or market-research an idea — 'is this a good idea', 'should I build this', 'will people pay for X', 'does this have a market', 'validate my startup idea', 'who is already doing this' — even if they never say ScrapingDog or MCP. Also use for narrower slices of the same work: 'find competitors for X', 'how much does X cost people already', 'what are people complaining about in [niche]', or 'check demand for [product]'."
---
...
- **Who it's for**: the specific user, not "everyone" (say "freelance video editors," not
  "creators").
...
| Everything else (default SaaS/app/consumer) | n/a | The universal set below covers it |
...
- [ ] Every number, quote, price, and competitor name in your notes traces to a specific tool call,
      not memory.
- [ ] Every category that came up empty says so explicitly rather than being padded or skipped.
- [ ] The verdict band you're about to write matches the score `score_idea.py` actually returned.
- [ ] Every source you're about to cite is in your notes with its URL.
...
- **Never invent**: a competitor, quote, price, hiring number, or AI-assistant answer. If you can't
  find it, the report says so: a fabricated data point is worse than a gap, because it gets acted on.
  The tell: a data point in the report that doesn't trace back to a specific tool call in your
  research notes.
- **Score isn't the report**: don't let the score become the whole report. The evidence underneath it
  is what makes this useful; the number is just the summary.
- **Quote, don't dump**: keep pain-point quotes short and attributed (platform + link), not
  screenshots of entire threads.
- **Match tools to type**: don't reach for every tool on every idea just because it exists. The
  type-classification table in Step 1 exists so a B2C mobile app doesn't get a pointless
  `google_patents` call and a physical product doesn't get skipped on `amazon_reviews`. The tell: a
  research step that doesn't map to anything in the Step 1 type table. Breadth should track the idea,
  not the toolbox.
