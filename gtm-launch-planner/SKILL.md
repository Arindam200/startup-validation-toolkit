...
description: Turns a validated idea into an actual go-to-market plan using the ScrapingDog MCP server — maps buyer-persona titles from live job postings, finds where the audience already gathers (YouTube content gaps, X communities, local listings), checks what ChatGPT and Google's AI answer already recommend for this problem (a discovery channel a plain search skill can't see at all), benchmarks pricing against real competitor data, and turns competitor weaknesses into a positioning statement. Third and final stage of a three-skill pipeline: `startup-idea-validator` decides whether to build it, `competitor-teardown` studies the named rivals in depth, and this skill answers "okay, it's validated — now how do I actually sell it." Use this when the user has (or believes they have) a validated idea and asks how to launch, market, position, price, or find first customers for it — "how do I sell this", "who's my audience and where do I find them", "how should I position this against [competitor]", "what channels should I use", "help me plan my launch", or "write me a go-to-market plan". Don't use this for the initial "is this a good idea" question — that's `startup-idea-validator`; this assumes the idea already cleared that bar (or the user is proceeding anyway and wants a plan regardless).
---
...
- Every recommendation should trace to a specific finding from Step 2 — a channel or persona claim
  with no cited evidence is exactly the generic advice this skill exists to avoid.
- Don't recommend a channel just because a tool exists for it. If `youtube_search` turns up nothing
  relevant, that's evidence *against* YouTube as a channel here, not a reason to skip reporting it.
- The AI-visibility findings are a genuine differentiator for this skill — don't bury them as a minor
  aside. If nobody currently owns the AI-assistant answer for this problem, that's headline material.
- Never invent a job-posting count, competitor price, or AI-assistant answer. Say "not found" plainly.
